...
using PowerChallenge;
    ...
    public class bgThread2 : Component
    {
        ...
        /// <summary>
        /// the thread sleep time between blocking calls
        /// </summary>
        public int iSleepTime = 1000;

        private Queue _queueArgs;
        ...
        public bgThread2(Queue queueArgs)
        {
            bgWnd = new bgThread2WndProc(this);
            this._queueArgs = queueArgs;
            myThread = new Thread(myThreadStart);
            bRunThread = true;
            myThread.Start();
        }
        ...
        private void myThreadStart()
        {
            ...
            try
            {
                do
                {
                    ...
                    if (this._queueArgs.Count > 0)
                    {
                        lock (this._queueArgs.SyncRoot)
                        {
                            ActionsClass.myAction sAction = (ActionsClass.myAction)this._queueArgs.Dequeue();
                            //do the action
                            Thread.Sleep(100);
                            sAction.dtEnd = DateTime.Now;
                            sMsg = sAction.actionType.ToString()+" finished";
                            id = (int)sAction.actionType;
                        }
                        SendData(bgWnd.Hwnd, id, sMsg);
                    }
